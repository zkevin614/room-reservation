...
| Admin CRUD for sites and rooms | `Admin::SitesController` + nested `Admin::RoomsController`; index/show/create/edit/destroy; active toggle; delete blocked when reservations exist; Sites nav link; README scope updated | **Accepted**. |
| GitHub Actions CI failing (lint, scan_js, test, system-test) | RuboCop array spacing; restored importmap (`bin/importmap`, pins, Turbo/Stimulus); `generates_token_for :password_reset`; password tests match `<p>` flash markup; `test/system` so `test:system` has a path | **Accepted**. |